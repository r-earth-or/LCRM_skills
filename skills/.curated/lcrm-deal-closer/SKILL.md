---
name: lcrm-deal-closer
description: 先获取客户信息和跟进记录，判断产品匹配路径，输出针对性打单策略。支持售前数字员工和 Langtum 平台两条产品线。
---
...
- `lcrm-search` skill：获取客户商务记录（跟进历史）
...
如需参考跟进记录，调用 `lcrm-search` skill 获取该客户的商务记录：
- 脚本：`node scripts/search.mjs customer-business-records --customer-id "<customerId>"`
- 如不知道 customerId，先用：`node scripts/search.mjs customers --company-name "<客户名>" --limit 5`
